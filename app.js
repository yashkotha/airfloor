...
var DICT={
 en:{
  ...
  ph_search:"Area, district, city or 6-digit PIN code", ph_search_m:"Area, city or PIN code", btn_search:"Search", try:"Try:",
  ...
  aqs_is:"Air quality is", aqs_unit:"AQI (US)",
  ...
  chk_h:"Before you buy, ask", chk_lead:"The score points you to the right floors. These questions and papers help you weigh the building behind them. Carry this list to the site visit.", chk_tab_q:"Questions for the developer", chk_tab_d:"Documents to request", chk_disc:"General guidance for apartment buyers in India, not legal advice.",
  chk_q1_t:"Distance from the road", chk_q1_b:"How many metres set the tower back from the nearest busy road, and which way do the bedrooms face?",
  chk_q2_t:"Generator and basement exhaust", chk_q2_b:"Where do the diesel generator and car-park exhausts vent, and which floors sit in their path?",
  chk_q3_t:"Cross ventilation", chk_q3_b:"Is every flat drawn for cross ventilation, or do some rooms open to only one side?",
  chk_q4_t:"Sewage plant location", chk_q4_b:"Where is the sewage treatment plant, and how far is it from the lowest homes?",
  chk_q5_t:"Windows that open", chk_q5_b:"On the floor you are considering, do the windows truly open, or is the tower sealed for full-time air conditioning?",
  chk_q6_t:"What sits within a kilometre", chk_q6_b:"Is there a highway, a factory, a landfill, or a years-long construction site close by?",
  chk_q7_t:"What comes up next", chk_q7_b:"What is approved for the empty plots around this project over the next five years?",
  chk_q8_t:"Fresh-air upkeep", chk_q8_b:"If higher floors rely on mechanical fresh-air units, who changes the filters and how often?",
  chk_d1_t:"Sanctioned building plan", chk_d1_b:"The plan approved by the local authority, showing the tower's real position, height and setbacks.",
  chk_d2_t:"Occupancy Certificate", chk_d2_b:"The OC confirms the building is legally fit to live in; do not take possession without it.",
  chk_d3_t:"RERA registration", chk_d3_b:"The project's RERA number and page, where approvals, timelines and complaints stay on public record.",
  chk_d4_t:"Pollution board consent", chk_d4_b:"The State Pollution Control Board consent to operate the diesel generators and the sewage plant.",
  chk_d5_t:"Environmental clearance", chk_d5_b:"For larger projects, the clearance and its conditions on air, dust and green cover.",
  chk_d6_t:"Fire safety NOC", chk_d6_b:"The fire department no-objection certificate for the tower's height and its escape routes.",
  chk_d7_t:"Sewage plant test reports", chk_d7_b:"Recent reports showing the plant meets discharge norms, so odour and fumes stay in check.",
  chk_d8_t:"Structural stability certificate", chk_d8_b:"Proof of structural soundness, which matters most for a resale or an older building.",
  map_h:"Live air across India", map_lead:"The current US AQI at fifty cities, refreshed every five minutes from the Copernicus model. Tap a city to read its air.", map_updated:"Updated {time}", map_stale:"Showing the last reading. Could not refresh just now.", map_hint:"Lower is cleaner. Colours follow the US AQI scale.", map_na:"no data",
  map_aria:"Live air quality across Indian cities", chk_aria:"Buyer guidance",
...
function renderAdvice(){
  ...
  var box=el('adviceBox'), b=adviceBand(), aqi=effAqi();
  var key=b+'.'+advPersona;
  var t=b?((ADVICE[LANG]&&ADVICE[LANG][key])||ADVICE.en[key]||''):'';
  /* never show an empty advice box: hide it unless we have real guidance to show */
  if(!b || aqi==null || STATE.floors==null || !t){ if(box) box.hidden=true; return; }
  if(box) box.hidden=false;
  t=t.replace(/\{lo\}/g,bandLo).replace(/\{hi\}/g,bandHi).replace(/\{aqi\}/g,Math.round(aqi));
  body.textContent=t;
  var lb=el('adviceLbl'); if(lb){ var pk={kids:'p_kids',elder:'p_elder',asthma:'p_asthma',run:'p_run'}[advPersona]; lb.textContent=T(pk)+' | '+T(catFor(aqi).k); }
...
function initChecklist(){
  var tabs=Array.prototype.slice.call(document.querySelectorAll('.chk-tab')); if(!tabs.length) return;
  function activate(tb, focus){
    var key=tb.getAttribute('data-panel');
    tabs.forEach(function(t){ var on=t===tb; t.classList.toggle('active',on); t.setAttribute('aria-selected',on?'true':'false'); t.setAttribute('tabindex',on?'0':'-1'); });
    document.querySelectorAll('.chk-panel').forEach(function(p){ p.hidden = p.getAttribute('data-panel')!==key; });
    if(focus) tb.focus();
  }
  tabs.forEach(function(tb,i){
    tb.addEventListener('click', function(){ activate(tb,false); });
    tb.addEventListener('keydown', function(e){
      ...
      if(ni>=0){ e.preventDefault(); activate(tabs[ni],true); }
    });
  });
}
...
function makeCard(nameKey, valTxt, unit, sub, pillKey, pillClass){
  ...
  if(pillClass){ c.classList.add('has-accent'); var ac=document.createElement('span'); ac.className='dcard-accent '+pillClass.replace('p-','acc-'); c.appendChild(ac); }
...
/* rough 3-band severity per pollutant, from WHO 2021 guideline levels (ug/m3); a guide, not an index */
function pollCat(kind,v){
  if(v==null) return {k:null,c:null};
  var T2={pm25:[15,35],pm10:[45,100],no2:[25,50],o3:[100,160],co:[4000,10000]}[kind];
  if(!T2) return {k:null,c:null};
  if(v<=T2[0]) return {k:'cat_good',c:'p-good'};
  if(v<=T2[1]) return {k:'cat_mod',c:'p-mod'};
  return {k:'cat_unh',c:'p-bad'};
}
/* piecewise position (0..1) of an AQI value on the 6-band scale */
function aqiScalePos(v){
  var bp=[0,50,100,150,200,300,500]; v=Math.max(0,Math.min(500,v||0));
  for(var i=0;i<bp.length-1;i++){ if(v<=bp[i+1]){ return (i+(v-bp[i])/(bp[i+1]-bp[i]))/6; } }
  return 1;
}
function renderAqiScale(){
  var box=el('aqiScale'); if(!box) return;
  var aqi=effAqi();
  if(aqi==null){ box.hidden=true; return; }
  box.hidden=false;
  el('aqsVal').textContent=Math.round(aqi);
  var cat=catFor(aqi);
  var cv=el('aqsCat'); cv.textContent=T(cat.k); cv.style.color=aqiColor(aqi);
  el('aqsMarker').style.left=(aqiScalePos(aqi)*100)+'%';
}
function renderAQ(){
  renderAqiScale();
  ...
  var pc=pollCat('pm25',pm25);
  var rows=[
    ['aq_pm25', pm25, 'µg/m³', [whoMult!=null?(whoMult.toFixed(1)+' '+T('whox')):'', tm].filter(Boolean).join(' | '), pc.k, pc.c],
    ['aq_pm10', a.pm10, 'µg/m³', tm, pollCat('pm10',a.pm10).k, pollCat('pm10',a.pm10).c],
    ['aq_no2', a.nitrogen_dioxide, 'µg/m³', tm, pollCat('no2',a.nitrogen_dioxide).k, pollCat('no2',a.nitrogen_dioxide).c],
    ['aq_o3', a.ozone, 'µg/m³', tm, pollCat('o3',a.ozone).k, pollCat('o3',a.ozone).c],
    ['aq_co', a.carbon_monoxide, 'µg/m³', tm, pollCat('co',a.carbon_monoxide).k, pollCat('co',a.carbon_monoxide).c]
  ];
...
/* the full search placeholder clips on phones; use a shorter one on narrow screens */
function setSearchPlaceholder(){ var q=el('q'); if(!q) return; q.setAttribute('placeholder', T(window.innerWidth<=560 ? 'ph_search_m' : 'ph_search')); }
...
function initPreloader(){
  var pl=el('preloader'); if(!pl) return;
  var reduce=false; try{ reduce=matchMedia('(prefers-reduced-motion: reduce)').matches; }catch(e){}
  var mark=pl.querySelector('.pl-mark'), sub=pl.querySelector('.pl-sub');
  var root=document.documentElement;
  root.classList.add('pl-active');
  var gone=false;
  function remove(){ if(gone) return; gone=true; root.classList.remove('pl-active'); if(pl.parentNode) pl.parentNode.removeChild(pl); }
  if(reduce){ pl.classList.add('pl-fadeall'); setTimeout(remove,480); return; }
  var done=false;
  function finish(){
    ...
    if(target && mark){
      var s=mark.getBoundingClientRect(), t=target.getBoundingClientRect();
    ...
    var bg=pl.querySelector('.pl-bg'); if(bg) bg.classList.add('pl-fade');   /* reveal the site */
    if(sub) sub.classList.add('pl-fade');
    setTimeout(function(){ root.classList.remove('pl-active'); }, 820);  /* hand off exactly as the clone lands */
    setTimeout(remove, 900);
  }
  setTimeout(finish, 1300);                 /* let the progress bar fill first */
  setTimeout(finish, 5000);                 /* hard safety cap */
}
function init(){
  initPreloader();
  startDitherDrift();
  ...
  updateGates();
  detectLocation();
  var rt, lastW=window.innerWidth; window.addEventListener('resize', function(){ clearTimeout(rt); rt=setTimeout(function(){ positionGlider(); setSearchPlaceholder(); if(window.innerWidth!==lastW){ lastW=window.innerWidth; renderDither(ditherPhase); redrawCharts(); } },160); });
